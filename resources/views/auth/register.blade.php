<x-guest-layout>
    <div class="space-y-6">
        <div>
            ...
            <p class="mt-2 text-sm text-slate-500">Registra tus datos para acceder a la plataforma.</p>
        </div>

        <form method="POST" action="{{ route('register') }}" class="space-y-5" x-data="passwordField()">
            @csrf

            <div>
                <label for="name" class="app-label">Nombre</label>
                <input id="name" class="app-input" type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" />
                <x-input-error :messages="$errors->get('name')" class="mt-2" />
            </div>

            <div>
                <label for="email" class="app-label">Correo electrónico</label>
                <input id="email" class="app-input" type="email" name="email" value="{{ old('email') }}" required autocomplete="username" />
            ...
            <div class="flex items-center justify-between gap-4">
                <a class="text-sm font-medium text-[#960018] transition hover:text-[#7b0014]" href="{{ route('login') }}">
                    Ya tengo una cuenta
                </a>

                <button class="app-button" type="submit" style="color: #ffffff !important;">
                    Registrarme
                </button>
            </div>
        </form>
