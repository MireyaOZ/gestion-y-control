<x-guest-layout>
    <div class="space-y-6">
        <div>
            <h1 class="text-2xl font-semibold text-slate-900">Confirmar contraseña</h1>
            <p class="mt-2 text-sm text-slate-500">Esta es un área protegida. Confirma tu contraseña antes de continuar.</p>
        </div>

        <form method="POST" action="{{ route('password.confirm') }}" class="space-y-5" x-data="passwordField()">
            @csrf

            <div>
                <label for="password" class="app-label">Contraseña</label>
                <div class="relative">
                    <input id="password" class="app-input pr-12" :type="showPassword ? 'text' : 'password'" name="password" required autocomplete="current-password" />
